...
class sys_API0002:
    ...
    @func_set_timeout(320)#设定函数超执行时间 
    def Modify_servertime(self, to_modify_datetime, job, dict):
        ...
        try:
            ...
            if dict['logInfo']['job'] == 'Modify_servertime':   #修复服务器时间组件
                times = 0
            elif dict['logInfo']['job'] in ('nb', 'complex_scenes'):   #新契约出单组件+复杂场景组件
                times = 1
            elif dict['logInfo']['job'] == 'single_invalidation_bacth':
                times = 1
                node = '到宽限期外'
            elif dict['logInfo']['job'] in ('renew' ,'clm'):   #续期组件
                times = 2
            else:
                times = 0
            #记录日志
            ...
            if (dict['logInfo']['job'] == 'Modify_servertime' and old_servicetime[0:13] != to_modify_datetime[0:13]) or (dict['logInfo']['job'] != 'Modify_servertime' and old_servicetime[0:10] != to_modify_datetime[0:10]): 
                ...
                new_servicetime = API0000_diy().modify_date1(to_modify_datetime , env_name)   # 修改成功后，获取当期服务器时间 并转换为正常格式：年-月-日
...
if __name__ == "__main__":
    ...
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            to_modify_datetime = '2021-10-01'   #修改时间为 
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'public': {'env_name':env_name, 'to_modify_datetime':to_modify_datetime}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'','job':'Modify_servertime'}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    ...
                    print ('修改后时间为：' + dict['public']['to_modify_datetime']).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否修改成功：成功,,环境：%s,,修改前时间为：%s,,修改后时间为：%s,,耗时：%s" % (env_name, dict['public']['old_servicetime'], str(dict['public']['to_modify_datetime']),API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否修改成功：不成功,,不成功原因：%s,,修改时间为：%s,,环境：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, dict['public']['to_modify_datetime'],API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
