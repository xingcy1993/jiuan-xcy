...
class sys_API0021:
    ...
    def additional_premium(self, apply_date, addInvestAmount, dict):
        """追加保费:
               step1:保全录入
               step2:保全复核+保全收费（转进账单）
               step3:定义轨迹信息+检查点信息::   
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '追加保费_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            resultData = []
            #step1:保全录入
            rs = self.pa_entry(apply_date, addInvestAmount, dict) 
            resultData = rs[0]
            dict = rs[1]
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            #step2:保全复核+保全收费（转进账单）
            rs = self.pa_review(apply_date, addInvestAmount, dict)  
            payment = rs[0]   #收费费金额
            dict = rs[1]
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            end_time = datetime.datetime.now()   #结束时间
            #记录日志
            logging.warning(API0000_diy().text_conversion('追加保费_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
    ...
    def pa_entry(self, apply_date, addInvestAmount, dict):
        """step1:保全录入
                1.保全录入用户登录系统
                2.保全申请接口
                3.保全录入-产品信息详细
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-追加保费_保全录入-产品信息详细'  # 节点
            message = ''
            #dict获取数据+定义数据
            loan_dict = []   #定义loan_dict。存入产品的追加保费数据信息          
            #调用原子脚本
            response = API0000_pa().queryCsPolicyProductBeforeChange2(magicX, interface_ip, policyNo, changeId, partitionIndi, serviceCode) 
            #获取所需响应值
            is_success = response['isSuccess']
                #定义追加保费列表。并存入产品id、产品现价、产品追加保费限额；
            if is_success:
                message = ''
                csProductCals = response['resultData']
                for i in range(0, len(csProductCals)):
                    csProductCals[i]['investAmount'] = None
                    initPremiumRate = 0.01   #初始扣费率
                    csProductCals[i]['addInvestAmount'] = float(addInvestAmount)   #申请追加金额
                    csProductCals[i]['surrenderAmount'] = round(float(addInvestAmount) * (1-initPremiumRate), 2)   #实际追加金额
                    csProductCals[i]['chargeAmount'] = round(float(addInvestAmount) * initPremiumRate, 2)   #手续费
                    csProductCals[i]['riskChargeFee'] = None
                    csProductCals[i]['riskChargeCurr'] = None
                    csProductCals[i]['initPremiumRate'] = initPremiumRate
            else:
                message = response['message']
                csProductCals = ''
            assert is_success == True
            ...
            #节点初始化
            is_success = ''
            unit = '系统-追加保费_保全录入-支付方式及支付账户接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #节点初始化
            is_success = ''
            unit = '系统-追加保费_保全录入-追加保费提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            applyTime1 = applyTime + " 00:00:00"  #组装申请日期applyTime
            #调用原子脚本
            response = API0000_pa().entryConfirm_additional_premium(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime1, serviceCode, csProductCals, payMode, accountId, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
    ...
    @func_set_timeout(90)#设定函数超执行时间
    def pa_review(self, apply_date, addInvestAmount, dict):
        """step2:保全复核+保全收费（转进账单）:
                1.保全复核用户登录系统
                2.保全复核-保全公共工作池查询接口 
                3.保全复核-工作池领取任务接口
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-追加保费_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-追加保费_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
    ...
    def save_check_info(self, apply_date, payment, resultData, dict):
        ...
        try:
            ...
            for i in range(0, len(resultData)):
                ...
                #申请追加金额
                if resultData[i]['addInvestAmount'] == None:
                    addInvestAmount = 0
                else:
                    addInvestAmount = float(resultData[i]['addInvestAmount'])
                #手续费
                ...
                #每个产品追加保费检查点追加到product产品列表中
                product.append({'productCode':productCode, 'addInvestAmount':addInvestAmount, 'chargeAmount': chargeAmount, 'surrenderAmount':surrenderAmount}) 
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_additional_premium = {'apply_date':apply_date, 'product':product, 'payment':payment, 'msg': ''}   
                dict['check']['calc_additional_premium'].append(calc_additional_premium) 
                """2.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'additional_premium', 'trackTime':apply_date, 'trackData':{'product':product, 'payment':payment}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_additional_premium = {'apply_date':apply_date, 'product':'', 'payment':payment, 'msg': message}   
                dict['check']['calc_additional_premium'].append(calc_additional_premium) 
                """2.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'additional_premium', 'trackTime':apply_date, 'trackData':{'product':'', 'payment':payment}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088633856323968'   #保单号
            apply_date = '2024-03-01'   #保全申请日期
            addInvestAmount = '1000'    #追加保费金额
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','policyNo','apply_date','addInvestAmount','username_entry','username_review']
                ...
                addInvestAmount = rs[2][3]   #追加保费金额
                username_entry = rs[2][4]   #保全录入用户
                username_review = rs[2][5]   #保全复核用户
       #3.校验录入字段是否符合规范
        key_dict = {'env_name':env_name,'apply_date':apply_date,'addInvestAmount':addInvestAmount}   #录入字段中待校验字典
        if msg == '':
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo},'public': {'env_name':env_name, 'username_entry':username_entry, 'username_review':username_review, 'interface_ip':interface_ip, 'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'', 'job':job}, 'track_info': [], 'check':{'calc_additional_premium':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            dict = sys_API0021().additional_premium(apply_date, addInvestAmount, dict)
            assert dict['logInfo']['code'] == '1'   #断言是否异常
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否追加保费成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('追加保费金额为：' + addInvestAmount).decode('utf-8').encode('gb2312')
                    print ('耗时：' + API0000_diy().logger_count_time(begin_time, end_time)).decode('utf-8').encode('gbk')
                else:
                    print ('是否追加保费成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否追加保费成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,追加保费金额为：%s,,耗时：%s" % (env_name, policyNo, apply_date, addInvestAmount, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否追加保费成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
