...
class sys_API0029:
    ...
    def agreement_tb(self, apply_date, agreementSurrenderAmount, dict):
        """系统协议退保:
               step1:保全录入
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '协议退保_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            payment = 0
            #保全录入
            rs = self.pa_entry(apply_date, agreementSurrenderAmount, dict)  
            agreement_tb_dict = rs[0]
            dict = rs[1]
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            #保全审核复核+收付费
            dict = self.pa_review_again(apply_date, dict) 
            payment = rs[0]
            dict = rs[1]
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            end_time = datetime.datetime.now()   #结束时间
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '协议退保_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
            ...
            if dict['logInfo']['code'] == '1':
                dict = self.save_check_info(apply_date, payment, agreement_tb_dict, dict)
        return dict



    @func_set_timeout(60)#设定函数超执行时间
    def pa_entry(self, apply_date, agreementSurrenderAmount, dict):
        """step1:保全录入
                1.保全录入用户登录系统
                2.保全申请接口
                3.保全录入-客户受理信息接口
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全协议退保_保全录入-客户受理信息接口'  # 节点
            message = ''
            #调用原子脚本
           ...
           #节点初始化
            ...
            is_success = response['isSuccess']
                #定义贷款列表。并存入产品id、产品现价、产品贷款限额；
            if is_success:
                message = ''
                resultData = response['resultData']
                for i in range(0, len(resultData)):
                    resultData[i]['agreementSurrenderAmount'] = float(agreementSurrenderAmount)   #协议退保金额
                    resultData[i]['agreementSupplymentAmount'] = round((float(agreementSurrenderAmount)-float(resultData[i]['surrenderAmount'])), 2)   #协议补差金额
            else:
                message = response['message']
                resultData = ''
            # 加入断言
            ...
            #存入产品的协议退保数据信息
            agreement_tb_dict = []   #agreement_tb_dict。存入产品的协议退保数据信息 
            #组装定义agreement_tb_dict产品的协议退保数据信息 
            for i in range(0,len(resultData)):
                agreement_tb_dict.append({})
                agreement_tb_dict[i]['productCode'] = str(resultData[i]['productCode'])   #产品id
                agreement_tb_dict[i]['presentPrice'] = resultData[i]['presentPrice']   #产品现价
                agreement_tb_dict[i]['investAmount'] = resultData[i]['investAmount']   #万能账户价值
                agreement_tb_dict[i]['chargeAmount'] = resultData[i]['chargeAmount']   #手续费
                agreement_tb_dict[i]['loanAccountAmount'] = resultData[i]['loanAccountAmount']   #欠款金额
                agreement_tb_dict[i]['refundRenewPrem'] = resultData[i]['refundRenewPrem']   #退还续期保费
                agreement_tb_dict[i]['riskChargeFee'] = resultData[i]['riskChargeFee']   #退还未到期风险保险费
                agreement_tb_dict[i]['bonusAccount'] = resultData[i]['bonusAccount']   #红利累计生息账户
                agreement_tb_dict[i]['paidBonus'] = resultData[i]['paidBonus']   #已分红金额扣回
                agreement_tb_dict[i]['surrenderAmount'] = resultData[i]['surrenderAmount']   #应付金额
                agreement_tb_dict[i]['agreementSurrenderAmount'] = resultData[i]['agreementSurrenderAmount']    #协议退保金额
                agreement_tb_dict[i]['agreementSupplymentAmount'] = resultData[i]['agreementSupplymentAmount']    #协议补差金额


            """5.保全录入-保存协议退保金额接口"""
           #节点初始化
            is_success = ''
            unit = '系统-保全协议退保_保全录入-保存协议退保金额接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            for i in range(0, len(resultData)):
                data = resultData[i]
                #调用原子脚本
                response = API0000_pa().updateAgreementAmountByProduct(magicX, interface_ip, data) 
                #获取所需响应值
                is_success = response['success']
                    #定义贷款列表。并存入产品id、产品现价、产品贷款限额；
                if is_success:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全协议退保_保全录入-支付方式及支付账户接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全协议退保_保全录入-协议退保提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            applyTime = applyTime + " 00:00:00"  #组装申请日期applyTime
            sCsArgtSurrender = {"policyNo":policyNo,"surrenderReason":"1","liabBranch":"2","surrenderReasonDtl":"其他","liabBranchDtl":"无"}
            csProductCals = resultData
            #调用原子脚本
            response = API0000_pa().entryConfirm_agreement_tb(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime, serviceCode, contactNumber, telephone, sCsArgtSurrender, csProductCals, payMode, accountId, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
    ...
    def pa_review(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全协议退保_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全协议退保_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全协议退保_保全复核提交接口'  # 节点
            message = ''
            #调用原子脚本
            response = API0000_pa().csEffect(magicX, interface_ip, changeId, applyNo, serviceCode, partitionIndi, policyNo)
            #获取所需响应值
            if response['success'] == False and response['message'] == '此保全申请需要二次审核!':
                is_success = True
                message = ''
                is_pa_review_again = '1'
            ...
            dict['public']['magicX'] = magicX
            dict['public']['is_pa_review_again'] = is_pa_review_again   #记录是否审核复核标识
            dict['logInfo']['code'] = '1'   #记录无异常标识
    ...
    @func_set_timeout(60)#设定函数超执行时间
    def pa_review_again(self, apply_date, dict):
        """step3:保全审核复核：
            1.保全审核复核用户登录系统
            2.保全审核复核-保全公共工作池查询接口 
            3.保全审核复核-工作池领取任务接口
            4.保全审核复核提交接口
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-协议退保_保全审核复核用户登录系统'  # 节点
            message = ''
            #dict获取数据+定义数据
            env_name = dict['public']['env_name']   #环境名称
            interface_ip = dict['public']['interface_ip']   #接口ip
            username = dict['public']['username_review_again']    #保全审核复核用户
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            rs = API0000_sys_otherapi().login(env_name,username, interface_ip)
            #获取所需响应值
            ...
            # 加入断言
            assert is_success == True            
            if dict['public']['is_pa_review_again'] == '1':
                #记录日志
                logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '*保全审核复核_开始(预计耗时:10s)'))
                """2.保全审核复核-保全公共工作池查询接口"""
                #节点初始化
                is_success = ''
                unit = '系统-协议退保_保全审核复核-保全公共工作池查询接口'  # 节点
                message = ''
                #调用原子脚本
                response = API0000_pa().queryCsInfoList(magicX, interface_ip, policyNo) 
                #获取所需响应值
                ...
                #节点初始化
                is_success = ''
                unit = '系统-协议退保_保全审核复核-工作池领取任务接口'  # 节点
                message = ''
                #dict获取数据+定义数据
                changeIds = changeId   #组装changeIds
                #调用原子脚本
                ...
                end_time1 = datetime.datetime.now()   #结束时间
                #记录日志
                logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '*保全审核复核_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time1, end_time1)))
            else:
                pass   
            """保全收付费"""
    ...
    def save_check_info(self, apply_date, payment, agreement_tb_dict, dict):
        """step3:定义轨迹信息+检查点信息: 
                 1.定义轨迹信息
                 2.定义检查点信息
        """
        try: 
            product = []   #定义协议退保产品列表
            payment = 0   #定义总协议退保金额
            for i in range(0,len(agreement_tb_dict)):
                #产品代码
                productCode = str(agreement_tb_dict[i]['productCode'])
                #现金价值
                if agreement_tb_dict[i]['presentPrice'] == None:
                    presentPrice = 0
                else:
                    presentPrice = float(agreement_tb_dict[i]['presentPrice'])
                #万能账户价值   
                if agreement_tb_dict[i]['investAmount'] == None:
                    investAmount = 0
                else:
                    investAmount = float(agreement_tb_dict[i]['investAmount'])
                #手续费  
                if agreement_tb_dict[i]['chargeAmount'] == None:
                    chargeAmount = 0
                else:
                    chargeAmount = float(agreement_tb_dict[i]['chargeAmount'])
                #欠款余额  
                if agreement_tb_dict[i]['loanAccountAmount'] == None:
                    loanAccountAmount = 0
                else:
                    loanAccountAmount = float(agreement_tb_dict[i]['loanAccountAmount'])
                #退还续期保费    
                if agreement_tb_dict[i]['refundRenewPrem'] == None:
                    refundRenewPrem = 0
                else:
                    refundRenewPrem = float(agreement_tb_dict[i]['refundRenewPrem'])
                #退还未到期风险保险费
                if agreement_tb_dict[i]['riskChargeFee'] == None:
                    riskChargeFee = 0
                else:
                    riskChargeFee = float(agreement_tb_dict[i]['riskChargeFee'])
                #红利累计生息账户
                if agreement_tb_dict[i]['bonusAccount'] == None:
                    bonusAccount = 0
                else:
                    bonusAccount = float(agreement_tb_dict[i]['bonusAccount'])
                #已分红金额扣回  
                if agreement_tb_dict[i]['paidBonus'] == None:
                    paidBonus = 0
                else:
                    paidBonus = float(agreement_tb_dict[i]['paidBonus'])
                #应付金额  
                if agreement_tb_dict[i]['surrenderAmount'] == None:
                    surrenderAmount = 0
                else:
                    surrenderAmount = float(agreement_tb_dict[i]['surrenderAmount'])
                #协议退保金额
                if agreement_tb_dict[i]['agreementSurrenderAmount'] == None:
                    agreementSurrenderAmount = 0
                else:
                    agreementSurrenderAmount = float(agreement_tb_dict[i]['agreementSurrenderAmount'])
                #协议补差金额  
                if agreement_tb_dict[i]['agreementSupplymentAmount'] == None:
                    agreementSupplymentAmount = 0
                else:
                    agreementSupplymentAmount = float(agreement_tb_dict[i]['agreementSupplymentAmount'])

                #每个产品协议退保检查点追加到product协议退保产品列表中
                product.append({'productCode':productCode, 'presentPrice':presentPrice, 'investAmount':investAmount, 'chargeAmount':chargeAmount, 'loanAccountAmount':loanAccountAmount, 'refundRenewPrem':refundRenewPrem, 'riskChargeFee':riskChargeFee, 'bonusAccount':bonusAccount, 'paidBonus':paidBonus, 'surrenderAmount':surrenderAmount, 'agreementSurrenderAmount':agreementSurrenderAmount, 'agreementSupplymentAmount':agreementSupplymentAmount}) 
                #总协议退保金额
                payment = agreementSurrenderAmount
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_agreement_tb = {'apply_date':apply_date, 'product':product, 'payment':payment, 'msg': ''}   
                dict['check']['calc_agreement_tb'].append(calc_agreement_tb)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'agreement_tb', 'trackTime':apply_date, 'trackData':{'product':product, 'payment':payment}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_agreement_tb = {'apply_date':apply_date, 'product':'', 'payment':payment, 'msg': message}   
                dict['check']['calc_agreement_tb'].append(calc_agreement_tb)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'agreement_tb', 'trackTime':apply_date, 'trackData':{'product':'', 'payment':payment}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088604074560668'   #保单号
            apply_date = '2020-01-01'   #保全申请日期
            agreementSurrenderAmount = '10000'    #协议退保金额(需大于退保应付金额)
            username_entry = 'xcy1'   #保全录入用户名
            username_review = 'xcy2'   #保全复核用户名 
            username_review_again = 'xingcy1'   #保全复核审核用户名
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','policyNo','apply_date','agreementSurrenderAmount','username_entry','username_review','username_review_again']
                ...
                agreementSurrenderAmount = rs[2][3]   #协议退保金额
                username_entry = rs[2][4]   #保全录入用户
                username_review = rs[2][5]   #保全复核用户
                username_review_again = rs[2][6]   #保全复核审核用户名
       #3.校验录入字段是否符合规范
        key_dict = {'env_name':env_name,'apply_date':apply_date,'agreementSurrenderAmount':agreementSurrenderAmount}   #录入字段中待校验字典
        if msg == '':
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo,'main_risk_info':[],'sub_risk_info':[]},'public': {'env_name':env_name, 'username_entry':username_entry, 'username_review':username_review, 'username_review_again':username_review_again, 'interface_ip':interface_ip,'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'','job':''}, 'track_info': [], 'check':{'calc_agreement_tb':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            dict = sys_API0029().agreement_tb(apply_date, agreementSurrenderAmount, dict)
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否协议退保成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('协议退保金额为：' + agreementSurrenderAmount).decode('utf-8').encode('gb2312')
                    print ('耗时：' + API0000_diy().logger_count_time(begin_time, end_time)).decode('utf-8').encode('gbk')
                else:
                    print ('是否协议退保成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否协议退保成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,协议退保金额为：%s,,耗时：%s" % (env_name, policyNo, apply_date, agreementSurrenderAmount, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否协议退保成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
