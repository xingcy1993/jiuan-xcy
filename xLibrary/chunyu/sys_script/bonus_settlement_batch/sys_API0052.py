...
class sys_API0052:
    ...
    def bonus_settlement_batch(self, sendDate, dict):
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '分红险结息批处理开始(预计耗时:120s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            """step1.1:判断保单是否存在待分红险结息批处理"""
            is_success = ''
            message = ''
            unit = "系统-分红险结息批处理_判断保单是否存在待分红数据"   #节点
            #dict获取数据+定义数据
            ...
            is_success = ''
            message = ''
            unit = "系统-分红险结息批处理_验证录入用户是否可正常登录系统"   #节点
            #验证用户是否可正常登录系统
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*(space_times+1) + '*启用分红险结息批处理3005开始(预计耗时:60s)'))
            time.sleep(30)
            """0.登录系统"""
            #节点初始化
            is_success = ''
            unit = '系统-分红险结息批处理_登录系统'  # 节点
            message = ''
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-分红险结息批处理_启动分红险结息批处理'  # 节点
            message = ''
            ...
            for j in range(0,60):
                #节点初始化
                is_success = ''
                unit = '系统-分红险结息批处理_判断分红险结息批处理是否成功'  # 节点
                message = ''
                #dict获取数据+定义数据
                policyNo = dict['policy_info']['policyNo']
                #调用原子脚本:
                allocate_status = ''   #初始化分红险结息状态
                sql2 = "select trans_amount from t_pa_acc_trans_list where deal_time='" + sendDate + "' and ACCOUNT_TYPE='06' and trans_code='12' and policy_no='" + policyNo + "';"
                #调用原子脚本    
                cursor = API0000_diy().db_conf(env_name,'wift_pa')
                cursor.execute(sql2)
                ...
                if len(result2) > 0:
                    is_success = True
                    message = ''
                    trans_amount = result2[0][0]   #分红险结息金额
                    break
                else:
                    is_success = False
                    message = '保单分红未结息成功！'
                    trans_amount = 0   #分红险结息金额
                    time.sleep(2)
            #断言
            assert is_success == True
            end_time1 = datetime.datetime.now()   #结束时间
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*(space_times+1) + '*启用分红险结息批处理3005结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time1, end_time1)))
            """step4:关闭批处理"""
            begin_time2 = datetime.datetime.now()   #开始时间
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*(space_times+1) + '*关闭分红险结息批处理3005开始(预计耗时:60s)'))

            #节点初始化
            is_success = ''
            unit = '系统-关闭分红险结息批处理'  # 节点
            message = ''
            #（1）关闭续期预收费用记录表接口
            batchSystem = 'PA'
            jobId = '3005'
            ...
            dict['public']['magicX'] = ''
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*(space_times+1) + '*关闭分红险结息批处理3005结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time2, end_time2)))

            """step5:定义轨迹信息+检查点信息"""            
            """1.定义检查点信息"""
            calc_bonus_settlement_batch = {}
            #分红险结息信息存入dict['check']['calc_bonus_settlement_batch']中
            calc_bonus_settlement_batch['apply_date'] = sendDate   #分红险结息日期
            calc_bonus_settlement_batch['payment'] = float(trans_amount)   #分红险结息金额
            calc_bonus_settlement_batch['msg'] = ''   
            dict['check']['calc_bonus_settlement_batch'].append(calc_bonus_settlement_batch)
            """2.定义轨迹信息"""
            track_info = {'trackType':'bonus_settlement_batch', 'trackTime':sendDate, 'trackData':{'payment':float(trans_amount)}, 'msg': ''}
            dict['track_info'].append(track_info)   #存入轨迹信息
            #存入dict
            dict['logInfo']['code'] = '1'
            end_time = datetime.datetime.now()   #结束时间
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '分红险结息批处理结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
            ...
            calc_bonus_settlement_batch = {}
            #分红险结息信息存入dict['check']['calc_bonus_settlement_batch']中
            calc_bonus_settlement_batch['apply_date'] = sendDate   #新契约收费金额
            calc_bonus_settlement_batch['payment'] = ''   #新契约收费金额
            calc_bonus_settlement_batch['msg'] = msg   #新契约收费金额
            dict['check']['calc_bonus_settlement_batch'].append(calc_bonus_settlement_batch)
            """2.定义轨迹信息"""
            track_info = {'trackType':'bonus_settlement_batch', 'trackTime':sendDate, 'trackData':{'payment':''}, 'msg': message}
            dict['track_info'].append(track_info)   #存入轨迹信息 
        finally:
...
if __name__ == "__main__":  
    try:
        ...
        job = 'bonus_settlement_batch'   #场景名
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088615306063868'   #保单号
            sendDate = '2022-01-02'   #跑批日期
       ...
       #3.校验录入字段是否符合规范
        ...
        #4.定义dict
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo},'public': {'env_name':env_name, 'username_entry':username_entry, 'interface_ip':interface_ip, 'sendDate':sendDate, 'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'', 'job':job}, 'track_info': [], 'check':{'calc_bonus_settlement_batch':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        #6.执行脚本
        if is_run == True:
            dict = sys_API0052().bonus_settlement_batch(sendDate, dict)
        ...
        try:
            ...
            if 'win' in sys.platform:   #windows系统平台
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "保单分红险结息是否成功：成功,,环境：%s,,保单号：%s,,分红险结息日期：%s,,耗时：%s" % (env_name, dict['policy_info']['policyNo'], sendDate, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "保单分红险结息是否成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,分红险结息日期：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, dict['policy_info']['policyNo'], sendDate, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
