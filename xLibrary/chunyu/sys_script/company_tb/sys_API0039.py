...
class sys_API0039:
    ...
    def company_tb(self, apply_date, cancelContractType, cancelReason, cancelComment, dict):
        """系统公司解约:
               step1:保全录入
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '公司解约_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            payment = 0
            #保全录入
            rs = self.pa_entry(apply_date, cancelContractType, cancelReason, cancelComment, dict)  
            company_tb_dict = rs[0]
            dict = rs[1]
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            #保全复核+收付费
            rs = self.pa_review(apply_date, cancelContractType, dict) 
            payment = rs[0]
            dict = rs[1]
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            end_time = datetime.datetime.now()   #结束时间
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '公司解约_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
            ...
            if dict['logInfo']['code'] == '1':
                dict = self.save_check_info(apply_date, payment, company_tb_dict, dict)
        return dict



    @func_set_timeout(60)#设定函数超执行时间
    def pa_entry(self, apply_date, cancelContractType, cancelReason, cancelComment, dict):
        """step1:保全录入
                1.保全录入用户登录系统
                2.保全申请接口
                3.保全录入-客户受理信息接口
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全公司解约_保全录入-客户受理信息接口'  # 节点
            message = ''
            #调用原子脚本
           ...
           #节点初始化
            ...
            #存入产品的公司解约数据信息
            company_tb_dict = []   #company_tb_dict。存入产品的公司解约数据信息 
            #组装定义company_tb_dict产品的公司解约数据信息 
            for i in range(0,len(resultData)):
                company_tb_dict.append({})
                company_tb_dict[i]['productCode'] = str(resultData[i]['productCode'])   #产品id
                company_tb_dict[i]['productName'] = resultData[i]['productName']   #险种名称
                company_tb_dict[i]['liabilityState'] = resultData[i]['liabilityState']   #险种状态
                company_tb_dict[i]['sumAssured'] = str(resultData[i]['sumAssured'])   #保额
                company_tb_dict[i]['totalPremAf'] = str(resultData[i]['totalPremAf'])   #保费
                company_tb_dict[i]['presentPrice'] = resultData[i]['presentPrice']   #现金价值
                company_tb_dict[i]['investAmount'] = resultData[i]['investAmount']   #账户价值
                company_tb_dict[i]['chargeAmount'] = resultData[i]['chargeAmount']   #手续费
                company_tb_dict[i]['loanAccountAmount'] = resultData[i]['loanAccountAmount']   #欠款金额
                company_tb_dict[i]['refundRenewPrem'] = resultData[i]['refundRenewPrem']   #退还续期保费
                company_tb_dict[i]['cbSbAccount'] = resultData[i]['cbSbAccount']   #累计生息账号
                #company_tb_dict[i]['cbSbAccount'] = resultData[i]['cbSbAccount']   #风险保费金额
                company_tb_dict[i]['bonusAccount'] = resultData[i]['bonusAccount']   #红利累计生息账户
                company_tb_dict[i]['paidBonus'] = resultData[i]['paidBonus']   #已分红金额扣回
                company_tb_dict[i]['repayAmount'] = resultData[i]['repayAmount']   #已支付金额
                company_tb_dict[i]['surrenderAmount'] = resultData[i]['surrenderAmount']   #应付金额
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全公司解约_保全录入-支付方式及支付账户接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全公司解约_保全录入-公司解约提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            applyTime = applyTime + " 00:00:00"  #组装申请日期applyTime
            csProductCals = resultData
            for i in range(0, len(csProductCals)):
                csProductCals[i]['cancelFlag'] = 'Y'
            #调用原子脚本
            response = API0000_pa().entryConfirm_company_tb(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime, serviceCode, contactNumber, telephone, cancelContractType, cancelReason, cancelComment, csProductCals, payMode, accountId, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
        ...
        return company_tb_dict, dict



    @func_set_timeout(60)#设定函数超执行时间
    def pa_review(self, apply_date, cancelContractType, dict):
        """step2:保全复核+保全收费（转进账单）
                1.保全复核用户登录系统
                2.保全复核-保全公共工作池查询接口 
                3.保全复核-工作池领取任务接口
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全公司解约_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全公司解约_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
    ...
    def save_check_info(self, apply_date, payment, company_tb_dict, dict):
        """step3:定义轨迹信息+检查点信息: 
                 1.定义轨迹信息
                 2.定义检查点信息
        """
        try: 
            product = []   #定义公司解约产品列表
            payment = 0   #定义总公司解约金额
            for i in range(0,len(company_tb_dict)):
                #产品代码
                productCode = str(company_tb_dict[i]['productCode'])
                #现金价值
                if company_tb_dict[i]['presentPrice'] == None:
                    presentPrice = 0
                else:
                    presentPrice = float(company_tb_dict[i]['presentPrice'])
                #万能账户价值   
                if company_tb_dict[i]['investAmount'] == None:
                    investAmount = 0
                else:
                    investAmount = float(company_tb_dict[i]['investAmount'])
                #手续费  
                if company_tb_dict[i]['chargeAmount'] == None:
                    chargeAmount = 0
                else:
                    chargeAmount = float(company_tb_dict[i]['chargeAmount'])
                #欠款余额  
                if company_tb_dict[i]['loanAccountAmount'] == None:
                    loanAccountAmount = 0
                else:
                    loanAccountAmount = float(company_tb_dict[i]['loanAccountAmount'])
                #退还续期保费    
                if company_tb_dict[i]['refundRenewPrem'] == None:
                    refundRenewPrem = 0
                else:
                    refundRenewPrem = float(company_tb_dict[i]['refundRenewPrem'])
                #累计生息账户
                if company_tb_dict[i]['cbSbAccount'] == None:
                    cbSbAccount = 0
                else:
                    cbSbAccount = float(company_tb_dict[i]['cbSbAccount'])
                #红利累计生息账户
                if company_tb_dict[i]['bonusAccount'] == None:
                    bonusAccount = 0
                else:
                    bonusAccount = float(company_tb_dict[i]['bonusAccount'])
                #已分红金额扣回  
                if company_tb_dict[i]['paidBonus'] == None:
                    paidBonus = 0
                else:
                    paidBonus = float(company_tb_dict[i]['paidBonus'])
                #已支付金额 
                if company_tb_dict[i]['repayAmount'] == None:
                    repayAmount = 0
                else:
                    repayAmount = float(company_tb_dict[i]['repayAmount'])
                #应付金额  
                if company_tb_dict[i]['surrenderAmount'] == None:
                    surrenderAmount = 0
                else:
                    surrenderAmount = float(company_tb_dict[i]['surrenderAmount'])

                #每个产品公司解约检查点追加到product公司解约产品列表中
                product.append({'productCode':productCode, 'presentPrice':presentPrice, 'investAmount':investAmount, 'chargeAmount':chargeAmount, 'loanAccountAmount':loanAccountAmount, 'refundRenewPrem':refundRenewPrem,'cbSbAccount':cbSbAccount,'bonusAccount':bonusAccount, 'paidBonus':paidBonus,'repayAmount':repayAmount, 'surrenderAmount':surrenderAmount}) 
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_company_tb = {'apply_date':apply_date, 'product':product, 'payment':payment, 'msg': ''}   
                dict['check']['calc_company_tb'].append(calc_company_tb)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'company_tb', 'trackTime':apply_date, 'trackData':{'product':product, 'payment':payment}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_company_tb = {'apply_date':apply_date, 'product':'', 'payment':payment, 'msg': message}   
                dict['check']['calc_company_tb'].append(calc_company_tb)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'company_tb', 'trackTime':apply_date, 'trackData':{'product':'', 'payment':payment}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088633856322968'   #保单号
            apply_date = '2021-10-10'   #保全申请日期
            cancelContractType = '3-解约且不退费'   #解约退费类型：1-解约且退还全部保费；2-解约且退还现金价值；3-解约且不退费
            cancelReason = '3-职业类别变更解约'    #解约原因：1-核保结论解约；2-客户重要信息变更解约；3-职业类别变更解约；4-其他
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','policyNo','apply_date','cancelContractType','cancelReason','username_entry','username_review']
                ...
                cancelContractType = rs[2][3]    #解约退费类型
                cancelReason = rs[2][4]    #解约原因
                username_entry = rs[2][5]    #保全录入用户
                username_review = rs[2][6]    #保全复核用户
       #3.校验录入字段是否符合规范
        key_dict = {'env_name':env_name,'apply_date':apply_date,'cancelContractType':cancelContractType,'cancelReason':cancelReason}   #录入字段中待校验字典
        if msg == '':
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo,'main_risk_info':[],'sub_risk_info':[]},'public': {'env_name':env_name, 'username_entry':username_entry, 'username_review':username_review, 'interface_ip':interface_ip,'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'','job':''}, 'track_info': [], 'check':{'calc_company_tb':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            dict = sys_API0039().company_tb(apply_date, cancelContractType[0], cancelReason[0], cancelComment, dict)
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否公司解约成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('解约退费类型:' + cancelContractType).decode('utf-8').encode('gb2312')
                    print ('解约原因：' + cancelReason).decode('utf-8').encode('gb2312')
                    print ('耗时：' + API0000_diy().logger_count_time(begin_time, end_time)).decode('utf-8').encode('gbk')
                else:
                    print ('是否公司解约成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                    print ('解约退费类型：' + cancelContractType).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否公司解约成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,解约退费类型：%s,,解约原因：%s,,耗时：%s" % (env_name, policyNo, apply_date, cancelContractType, cancelReason, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否公司解约成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,解约退费类型：%s,,解约原因：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, cancelContractType, cancelReason, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
