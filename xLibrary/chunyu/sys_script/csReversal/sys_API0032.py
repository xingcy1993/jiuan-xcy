...
class sys_API0032:
    ...
    def csReversal(self, apply_date, dict):
        """系统保全回退:
               step1:保全录入
               step2:保全审核+保全收费（转进账单）
               step3:定义轨迹信息+检查点信息:
        """
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '保全回退_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            #保全审核复核+收付费
            rs = self.pa_review_again(apply_date, dict) 
            payment = rs[0]
            dict = rs[1]
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            end_time = datetime.datetime.now()   #结束时间
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '保全回退_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
            #记录异常日志
    ...
    def pa_entry(self, apply_date, dict):
        ...
        try:
            ...
            assert is_success == True 
                #连接数据库
            unit = '系统-保全回退_保全录入-客户受理信息接口_连接数据库'
            ...
            for i in range(0,len(customerInfoList_old)):
                ...
                unit = '系统-保全回退_保全录入-客户受理信息接口_数据库查询客户证件止期'
                cursor.execute(sql)
                result = cursor.fetchall()
                unit = '系统-保全回退_保全录入-客户受理信息接口'
           ...
           #节点初始化
            is_success = ''
            unit = '系统-保全回退_保全录入-保全回退列表接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            csReversal_dict = []   #csReversal_dict。存入产品的保全回退数据信息   
            #调用原子脚本
            response = API0000_pa().csReversalChangeList(magicX, interface_ip, policyNo, changeId) 
            #获取所需响应值
            is_success = response['isSuccess']
                #定义保全回退列表。并存入产品id、产品现价、产品贷款限额；
            if is_success:
                message = ''
                ...
                message = response['message']
                resultData = ''
                #组装定义csReversal_dict产品的保全回退数据信息 
            csReversal_dict.append({})
            csReversal_dict[0]['serviceId'] = str(resultData[0]['serviceId'])   #保全项目代码
            csReversal_dict[0]['applyNo'] = str(resultData[0]['applyNo'])   #保全受理号
            # 加入断言
            assert is_success == True 
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全回退_保全录入-支付方式及支付账户接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #调用原子脚本
            response = API0000_pa().entryConfirm_csReversal(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime, serviceCode, payMode, accountId, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
    ...
    def pa_review(self, apply_date, dict):
        ...
        try:
            ...
            space_times = dict['public']['space_times'] + 1   #日志打印空格次数
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '*保全审核_开始(预计耗时:10s)'))
            is_pa_review_again = '0'   #是否保全审核复核标识。0-否；1-是。
            """1.保全审核用户登录系统"""
            #节点初始化
            is_success = ''
            unit = '系统-保全回退_保全审核用户登录系统'  # 节点
            message = ''
            #dict获取数据+定义数据
            env_name = dict['public']['env_name']   #环境名称
            interface_ip = dict['public']['interface_ip']   #接口ip
            username = dict['public']['username_review']    #保全审核用户
            #调用原子脚本
            rs = API0000_sys_otherapi().login(env_name,username, interface_ip)
            #获取所需响应值
            is_success = rs[0]
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全回退_保全审核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全回退_保全审核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '*保全审核_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time1, end_time1)))
        except Exception, e:
            is_success = False
            dict['logInfo']['code'] = '0'    #记录异常标识
            dict['logInfo']['err'] = unit + ':' + str(e)
    ...
    def pa_review_again(self, apply_date, dict):
        ...
        try:
            ...
            if dict['public']['is_pa_review_again'] == '1':
                ...
                #节点初始化
                is_success = ''
                unit = '系统-保全回退_保全审核复核-保全公共工作池查询接口'  # 节点
                message = ''
                #调用原子脚本
                response = API0000_pa().queryCsInfoList(magicX, interface_ip, policyNo) 
                #获取所需响应值
                ...
                #节点初始化
                is_success = ''
                unit = '系统-保全回退_保全审核复核-工作池领取任务接口'  # 节点
                message = ''
                #dict获取数据+定义数据
                changeIds = changeId   #组装changeIds
                #调用原子脚本
    ...
    def save_check_info(self, apply_date, payment, resultData, dict):
        ...
        try: 
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_csReversal = {'apply_date':apply_date, 'product':'', 'payment':payment, 'msg': ''}   
                dict['check']['calc_csReversal'].append(calc_csReversal)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'csReversal', 'trackTime':apply_date, 'trackData':{'product':'', 'payment':payment}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_csReversal = {'apply_date':apply_date, 'product':'', 'payment':payment, 'msg': message}   
                dict['check']['calc_csReversal'].append(calc_csReversal)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'csReversal', 'trackTime':apply_date, 'trackData':{'product':'', 'payment':payment}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088622620898968'   #保单号
            apply_date = '2031-07-24'   #保全申请日期
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','policyNo','apply_date','username_entry','username_review','username_review_again']
                ...
                username_review_again = rs[2][5]   #保全复核审核用户名
       #3.校验录入字段是否符合规范
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo, 'main_risk_info':[{'liability_state':''}], 'sub_risk_info':[{'liability_state':''}]},'public': {'env_name':env_name, 'username_entry':username_entry, 'username_review':username_review, 'username_review_again':username_review_again,'interface_ip':interface_ip}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'', 'job':job}, 'track_info': [], 'check':{'calc_csReversal':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            dict = sys_API0032().csReversal(apply_date, dict)
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否保全回退成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('是否保全回退成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否保全回退成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否保全回退成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
