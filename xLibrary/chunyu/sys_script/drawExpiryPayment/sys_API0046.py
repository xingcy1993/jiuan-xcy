...
class sys_API0046:
    ...
    def drawExpiryPayment(self, apply_date, dict):
        """系统满期领取:
               step1:保全录入
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '满期领取_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '满期领取_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
    ...
    def pa_entry(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全满期领取_保全录入-客户受理信息接口'  # 节点
            message = ''
            insured_name = ''
            #调用原子脚本
            ...
            assert is_success == True 
                #连接数据库
            unit = '系统-保全满期领取_保全录入-客户受理信息接口_连接数据库'
            ...
            for i in range(0,len(customerInfoList_old)):
                if customerInfoList_old[i]['customerRole'] == '01':
                    ...
                    unit = '系统-保全满期领取_保全录入-客户受理信息接口_数据库查询客户证件止期'
                    cursor.execute(sql)
                ...
                if customerInfoList_old[i]['customerRole'] == '02':
                    insured_name = customerInfoList_old[i]['name']
            # 加入断言
            assert is_success == True 
           ...
           #节点初始化
            is_success = ''
            unit = '系统-保全满期领取_保全录入-查询产品满期列表接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            drawExpiryPayment_dict = []   #drawExpiryPayment_dict。存入产品的满期领取数据信息   
            validateDate = ''
            #调用原子脚本
            ...
            is_success = response['isSuccess']
                #定义满期领取列表。并存入产品id、产品现价、产品满期领取限额；
            if is_success:
                message = ''
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全满期领取_保全录入-支付方式及支付账户接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            if is_success == True:
                if len(response['resultData']) > 0:
                    ...
                    is_success = ''
                    unit = '系统-保全满期领取_保全录入-增加支付方式及支付账户'  # 节点
                    message = ''
                    accountName = urllib.quote(insured_name.encode('utf-8'))
                    bankCode = '01'  #银行代码
                    bankAccount = '645562040697890137'   #银行账号
                    reserveMobile = '18988901738'   #银行预留手机号
                    #调用原子脚本
                    response = API0000_pa().insertAccount(magicX, interface_ip, changeId, partitionIndi, policyId, payMode, serviceCode, accountName, bankCode, bankAccount, reserveMobile)
                    if response['success'] == True and response['message'] == 'OK':
                        is_success = True
                        message = ''
                        #节点初始化
                        is_success = ''
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全满期领取_保全录入-满期领取提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #调用原子脚本
            response = API0000_pa().entryConfirm_drawExpiryPayment(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime, serviceCode, payMode, accountId, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
    ...
    def pa_review(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全满期领取_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全满期领取_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
    ...
    def save_check_info(self, apply_date, payment, resultData, dict):
        ...
        try: 
            ...
            for i in range(0,len(resultData)):
                ...
                #保费  
                if resultData[i]['totalPremAf'] == None:
                    totalPremAf = 0
                else:
                    totalPremAf = float(resultData[i]['totalPremAf'])
                #满期给付金额  
                if resultData[i]['expiryAmount'] == None:
                    expiryAmount = 0
                else:
                    expiryAmount = float(resultData[i]['expiryAmount'])
                #欠款余额  
                ...
                #满期应付金额
                if resultData[i]['expiryPayAmount'] == None:
                    expiryPayAmount = 0
                else:
                    expiryPayAmount = float(resultData[i]['expiryPayAmount'])

                #每个产品满期领取检查点追加到product满期领取产品列表中
                product.append({'productCode':productCode, 'sumAssured':sumAssured, 'totalPremAf':totalPremAf, 'expiryAmount':expiryAmount, 'loanAccountAmount':loanAccountAmount, 'cbSbAccount':cbSbAccount, 'bonusAccount':bonusAccount}) 
                #总满期领取金额
                payment = payment + expiryPayAmount
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_drawExpiryPayment = {'apply_date':apply_date, 'product':product, 'payment':payment, 'msg': ''}   
                dict['check']['calc_drawExpiryPayment'].append(calc_drawExpiryPayment)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'drawExpiryPayment', 'trackTime':apply_date, 'trackData':{'product':product, 'payment':payment}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_drawExpiryPayment = {'apply_date':apply_date, 'product':'', 'payment':payment, 'msg': message}   
                dict['check']['calc_drawExpiryPayment'].append(calc_drawExpiryPayment)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'drawExpiryPayment', 'trackTime':apply_date, 'trackData':{'product':'', 'payment':payment}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088622620904668'
            apply_date = '2031-07-28'   #保全申请日期
       ...
       #3.校验录入字段是否符合规范
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo},'public': {'env_name':env_name, 'username_entry':username_entry, 'username_review':username_review, 'interface_ip':interface_ip, 'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'', 'job':job}, 'track_info': [], 'check':{'calc_drawExpiryPayment':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            dict = sys_API0046().drawExpiryPayment(apply_date, dict)
            assert dict['logInfo']['code'] == '1'   #断言是否异常
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否满期领取成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('是否满期领取成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否满期领取成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否满期领取成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
