...
class sys_API0048:
    ...
    def initBonusMethod(self, apply_date, bonusPayMode, dict):
        """系统红利领取方式变更:
               step1:保全录入
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '红利领取方式变更_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            payment = 0
            #保全录入
            dict = self.pa_entry(apply_date, bonusPayMode, dict)  
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '红利领取方式变更_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
            ...
            if dict['logInfo']['code'] == '1':
                dict = self.save_check_info(apply_date, payment, bonusPayMode, dict)
        return dict


    @func_set_timeout(120)#设定函数超执行时间
    def pa_verification(self, bonusPayMode, dict):
        """验证录入的续保标识
        """
        try:
            begin_time = datetime.datetime.now()   #开始时间
            ...
            dict['public']['space_times'] = space_times
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '*验证录入的红利领取方式_开始(预计耗时:30s)'))
            """1.验证录入的续保标识"""
            #节点初始化
            is_success = ''
            unit = '验证录入的续保标识'  # 节点
            ...
            sql = "SELECT dividend_choice FROM t_pa_bonus_allocate WHERE POLICY_NO='" + POLICY_NO + "'"
            cursor.execute(sql)
            result = cursor.fetchall()
            
            if result == []:
                is_success = False
                message = '数据库未查询到此保单的红利领取方式！'
            else:
                if bonusPayMode == str(result[0][0]):
                    is_success = False
                    message = '经数据库查询，您选择的变更后保单下红利领取方式与录入的红利领取方式一致，不需要变更！'
                else:
                    is_success = True
                    message = ''
            # 加入断言
            ...
            end_time = datetime.datetime.now()   #结束时间
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '*验证录入的红利领取方式_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            is_success = False
    ...
    @func_set_timeout(120)#设定函数超执行时间
    def pa_entry(self, apply_date, bonusPayMode, dict):
        """step1:保全录入
                1.保全录入用户登录系统
                2.保全申请接口
                3.保全录入-客户受理信息接口
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_录入用户登录系统'  # 节点
            message = ''
            #dict获取数据+定义数据
            env_name = dict['public']['env_name']   #环境名称
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_保全录入-客户受理信息接口'  # 节点
            message = ''
            #调用原子脚本
            ...
            assert is_success == True 
                #连接数据库
            unit = '系统-保全红利领取方式变更_保全录入-客户受理信息接口_连接数据库'
            ...
            for i in range(0,len(customerInfoList_old)):
                if customerInfoList_old[i]['customerRole'] == '01':
                    ...
                    unit = '系统-保全红利领取方式变更_保全录入-客户受理信息接口_数据库查询客户证件止期'
                    cursor.execute(sql)
                    result = cursor.fetchall()
           ...
           #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_保全录入-保单红利领取方式查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据

            #调用原子脚本
            response = API0000_pa().initBonusMethod(magicX, interface_ip, apply_date, applyWay, applyType, serviceCode, policyNo, int(partitionIndi), changeId, applyNo, subSalesChannel, policyDeliveryMode) 
            #获取所需响应值
            is_success = response['success']
                #定义红利领取方式变更列表。并存入产品id、产品现价、产品红利领取方式变更限额；
            if is_success:
                message = ''
                listId = response['others']['listId']
            else:
                message = response['message']
                listId = ''
           # 加入断言
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_保全录入-支付方式及支付账户接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_保全录入-红利领取方式变更提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            applyTime = applyTime + " 00:00:00"  #组装申请日期applyTime
            flag = True
            #调用原子脚本
            response = API0000_pa().entryConfirm_initBonusMethod(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime, serviceCode, bonusPayMode, listId, payMode, accountId, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_保全录入-登出系统'  # 节点
            message = ''
            #调用原子脚本
    ...
    def pa_review(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_保全复核用户登录系统'  # 节点
            message = ''
            #dict获取数据+定义数据
            env_name = dict['public']['env_name']   #环境名称
            interface_ip = dict['public']['interface_ip']   #接口ip
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_保全复核提交接口'  # 节点
            message = ''
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全红利领取方式变更_保全复核-登出系统'  # 节点
            message = ''
            #调用原子脚本
            rs = API0000_sys_otherapi().logout(magicX,interface_ip)
            #获取所需响应值
    ...
    def save_check_info(self, apply_date, payment, bonusPayMode, dict):
        """step3:定义轨迹信息+检查点信息: 
                 1.定义轨迹信息
                 2.定义检查点信息
        """
        try: 
            payment = 0   #定义总红利领取方式变更金额
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_initBonusMethod = {'apply_date':apply_date, 'bonusPayMode':bonusPayMode, 'payment':payment, 'msg': ''}   
                dict['check']['calc_initBonusMethod'].append(calc_initBonusMethod)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'initBonusMethod', 'trackTime':apply_date, 'trackData':{'bonusPayMode':bonusPayMode, 'payment':payment}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_initBonusMethod = {'apply_date':apply_date, 'bonusPayMode':'', 'payment':payment, 'msg': message}   
                dict['check']['calc_initBonusMethod'].append(calc_initBonusMethod)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'initBonusMethod', 'trackTime':apply_date, 'trackData':{'bonusPayMode':'', 'payment':payment}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088627328722568'
            apply_date = '2021-10-10'   #保全申请日期
            bonusPayMode = '0-现金领取'   #红利领取方式。  0-现金领取；2-累计生息；3-转入万能账户
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','policyNo','apply_date','bonusPayMode','username_entry','username_review']
                ...
                bonusPayMode = rs[2][3]   #红利领取方式
                username_entry = rs[2][4]   #保全录入用户
                username_review = rs[2][5]   #保全复核用户
       #3.校验录入字段是否符合规范
        key_dict = {'env_name':env_name,'apply_date':apply_date,'bonusPayMode':bonusPayMode}   #录入字段中待校验字典
        if msg == '':
        ...
        bonusPayMode1 = bonusPayMode[0]
        #4.定义dict
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo, 'main_risk_info':[{'liability_state':''}], 'sub_risk_info':[{'liability_state':''}]},'public': {'env_name':env_name, 'username_entry':username_entry, 'username_review':username_review, 'interface_ip':interface_ip, 'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'', 'job':job}, 'track_info': [], 'check':{'calc_initBonusMethod':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            #验证录入的续保方式
            dict = sys_API0048().pa_verification(bonusPayMode1, dict)
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            #修改服务器时间
            dict = sys_API0002().Modify_servertime(apply_date, job, dict)
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            #保全-红利领取方式变更
            dict = sys_API0048().initBonusMethod(apply_date, bonusPayMode1, dict)
            assert dict['logInfo']['code'] == '1'   #断言是否异常
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否红利领取方式变更成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('红利领取方式变更为：' + bonusPayMode).decode('utf-8').encode('gb2312')
                    print ('耗时：' + API0000_diy().logger_count_time(begin_time, end_time)).decode('utf-8').encode('gbk')
                else:
                    print ('是否红利领取方式变更成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                    print ('红利领取方式变更为：' + bonusPayMode).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否红利领取方式变更成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,红利领取方式变更为：%s,,耗时：%s" % (env_name, policyNo, apply_date, bonusPayMode, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否红利领取方式变更成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,红利领取方式变更为：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, bonusPayMode, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
