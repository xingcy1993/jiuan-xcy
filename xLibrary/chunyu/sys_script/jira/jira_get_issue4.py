...
class jira_operation4:
    ...
    def jira_get_issue4(self, dict):
        try:
            ...
            tester_name = dict['tester_name']   #待分配人
            issues_list = []   #待分配人任务列表
            dict['issues_json']['json'] = []
            issue_assignee = ''

            #节点初始化
            is_success = ''
            message = ''
            unit = '节点：查询被分配人jira测试任务列表'  # 节点
            #定义
            if tester_name == '赵影娜':
            ...
            for j in range(0,len(issues_list)):
                ...
                dict['issues_json']['json'][-1]['issue_assignee'] = issue_assignee1
                ...
                #测试进展
                if issue.fields.customfield_10622 == None:
                    issue_progress = ''
                else:
                    issue_progress = str(issue.fields.customfield_10622)
                dict['issues_json']['json'][-1]['issue_progress'] = issue_progress
    ...
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                is_run = rs[0] 
                msg = rs[1]
                tester_name = rs[2][0]   #待分配人
        dict = {'execNo': execNo, 'tester_name': tester_name, 'issues_json':{'json':'','html':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':''}}
        #5.1定义logging文件路径
        ...
        #6.执行脚本
        dict = jira_operation4().jira_get_issue4(dict)
