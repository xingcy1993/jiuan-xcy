...
class sys_API0007:
    ...
    def loan(self, apply_date, loanApplyAmount, dict):
        """系统贷款:
               step1:保全录入
               step2:保全复核+保全收费（转进账单）
               step3:定义轨迹信息+检查点信息::   
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '贷款_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            payment = 0
            #step1:保全录入
            rs = self.pa_entry(apply_date, loanApplyAmount, dict)  
            resultData = rs[0]
            dict = rs[1]
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            time.sleep(3)
            #step2:保全复核+保全收费（转进账单）
            rs = self.pa_review(apply_date, loanApplyAmount, dict)  
            payment = rs[0]   #收费费金额
            dict = rs[1]
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            end_time = datetime.datetime.now()   #结束时间
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '贷款_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
    ...
    def pa_entry(self, apply_date, loanApplyAmount, dict):
        """step1:保全录入
                1.保全录入用户登录系统
                2.保全申请接口
                3.保全录入-产品信息详细
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全贷款_保全录入-产品信息详细'  # 节点
            message = ''
            #dict获取数据+定义数据
            loan_dict = []   #定义loan_dict。存入产品的贷款数据信息          
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全贷款_保全录入-支付方式及支付账户接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全贷款_保全录入-贷款提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            applyTime1 = applyTime + " 00:00:00"  #组装申请日期applyTime
            #调用原子脚本
            response = API0000_pa().entryConfirm_loan(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime1, serviceCode, loanApplyAmount, payMode, accountId, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
    ...
    def pa_review(self, apply_date, loanApplyAmount, dict):
        """step2:保全复核+保全收费（转进账单）:
                1.保全复核用户登录系统
                2.保全复核-保全公共工作池查询接口 
                3.保全复核-工作池领取任务接口
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全贷款_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全贷款_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
    ...
    def save_check_info(self, apply_date, payment, resultData, dict):
        ...
        try:        
            ...
            for i in range(0, len(resultData)):
                ...
                product.append({'productCode':productCode, 'presentPrice':presentPrice, 'investAmount': investAmount, 'loanAccountLimit':loanAccountLimit, 'loanRate':loanRate, 'loanAccountAmount':loanAccountAmount, 'sumAssured':sumAssured, 'stdPremBf':stdPremAf}) 
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_loan = {'loan_type':'loan', 'apply_date':apply_date, 'product':product, 'payment':payment, 'msg': ''}   
                ...
                #存入dict检查点中
                calc_loan = {'loan_type':'loan', 'apply_date':apply_date, 'product':product, 'payment':payment, 'msg': message}   
                dict['check']['calc_loan'].append(calc_loan) 
                """2.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'loan', 'trackTime':apply_date, 'trackData':{'product':product, 'payment':payment}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088759248281668'   #保单号
            apply_date = '2021-10-10'   #保全申请日期
            loanApplyAmount = '10000'    #保全贷款金额
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','policyNo','apply_date','loanApplyAmount','username_entry','username_review']
                ...
                loanApplyAmount = rs[2][3]   #贷款申请金额
                username_entry = rs[2][4]   #保全录入用户
                username_review = rs[2][5]   #保全复核用户
       #3.校验录入字段是否符合规范
        key_dict = {'env_name':env_name,'apply_date':apply_date,'loanApplyAmount':loanApplyAmount}   #录入字段中待校验字典
        if msg == '':
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否贷款成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('贷款金额为：' + loanApplyAmount).decode('utf-8').encode('gb2312')
                    print ('耗时：' + API0000_diy().logger_count_time(begin_time, end_time)).decode('utf-8').encode('gbk')
                else:
                    print ('是否贷款成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否贷款成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,贷款金额为：%s,,耗时：%s" % (env_name, policyNo, apply_date, loanApplyAmount, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否贷款成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
