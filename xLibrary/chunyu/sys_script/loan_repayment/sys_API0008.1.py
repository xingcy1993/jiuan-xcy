...
from xLibrary.chunyu.sys_script.query_policy_loan_info.sys_API0061 import sys_API0061
...
class sys_API0008:
    ...
    def loan_repayment(self, apply_date, repayment_amount, dict):
        """系统贷款还款:
               step1:保全录入
               step2:保全复核+保全收费（转进账单）
               step3:定义轨迹信息+检查点信息:    
        ...
        try:
            ...
            num_list = []
            #贷款数据处理，清除已结束贷款数据
            for i in range(0,len(dict['public']['list'])): #获取已结束贷款（本息和为0）的索引。
                if float(dict['public']['list'][i]['sum']) == 0:
                    num_list.append(i)
            for j in range(len(num_list)-1,-1,-1):   #倒叙按索引删除。
                del dict['public']['list'][j]
            assert dict['logInfo']['code'] == '1'   #断言是否异常

            #step2:拆分录入的部分贷款还款金额，及判断录入的贷款还款个数与待还款条数是否一致
            repayment_amount_list = repayment_amount.split(',')
            if len(repayment_amount_list) == len(dict['public']['list']):
                dict['logInfo']['code'] = '1'
            else:
                dict['logInfo']['code'] = '0'
                dict['logInfo']['msg'] = '经查询，贷款还款个数与待还款个数不一致！'
            assert dict['logInfo']['code'] == '1'   #断言是否异常

            #step3:保全录入
            rs = self.pa_entry(apply_date, repayment_amount_list, dict)  
            resultData = rs[0]
            dict = rs[1]
            assert dict['logInfo']['code'] == '1'   #断言是否异常
    ...
    def pa_entry(self, apply_date, repayment_amount_list, dict):
        """step1:保全录入
                1.保全录入用户登录系统
                2.保全申请接口
                3.保全录入-客户受理信息接口 
        ...
        try:
            ...
            if is_success:
                ...
                for z in range(0, len(csLoanAccounts)):
                    ...
                    csLoanAccounts[z]['policyPartialRmtAmount'] = str(repayment_amount_list[z])   #还款金额
                    if repayment_amount_list[z] == '0':
                        num_list.append(z)
                #删除录入贷款还款金额为0的记录
                for z1 in range(len(num_list)-1, -1, -1):
                    del csLoanAccounts[num_list[z1]]
            else:
                message = response['message']
                csLoanAccounts = ''
            # 加入断言
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088550236692768'
            apply_date = '2022-01-01'   #保全申请日期
            repayment_amount = '1000,1000'   #还款金额
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','policyNo','apply_date','repayment_amount','username_entry','username_review']
                ...
                repayment_amount = rs[2][3]   #还款金额
                username_entry = rs[2][4]   #保全录入用户
                username_review = rs[2][5]   #保全复核用户
        #还款金额去掉空格,中文逗号转换
        repayment_amount = repayment_amount.replace(' ', '')
        repayment_amount = repayment_amount.replace('，', ',')
       #3.校验录入字段是否符合规范
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo},'public': {'env_name':env_name,  'html_file_path':logging_filename, 'username_entry':username_entry, 'username_review':username_review, 'interface_ip':interface_ip, 'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'','job':job}, 'track_info': [], 'check':{'calc_loan':[]}, 'track_change':[]}
        #6.执行脚本
        if is_run == True:
            dict = sys_API0008().loan_repayment(apply_date, repayment_amount, dict)
            assert dict['logInfo']['code'] == '1'   #断言是否异常
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    ...
                    print ('录入的还款金额：' + repayment_amount).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否贷款还款成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,录入的还款金额：%s,,耗时：%s" % (env_name, policyNo, apply_date, repayment_amount, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否贷款还款成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,录入的还款金额：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, repayment_amount, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
