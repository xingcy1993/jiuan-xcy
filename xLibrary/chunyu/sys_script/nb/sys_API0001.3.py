...
class sys_API0001_3:
    ...
    def nb (self,env_name, dict):
        ...
        try:
            ...
            #dict中加入契约类型
            dict['nb_type'] = 'sys_API0001.3'   #契约类型：sys_API0001.3-新契约出单脚本（至复核结束）
            ...
            message = ''
            #调用原子脚本
            rs = API0000_sys_otherapi().logout(magicX, dict['public']['interface_ip'])
            #获取所需响应值
            magicX = ''
            is_success = rs[0]
            message = rs[1]
...
if __name__ == "__main__":   
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            username_entry = 'xcy1'   #新契约录入用户名
            username_review = 'xcy2'   #新契约复核用户名
        ...
        #6.执行脚本
        if is_run == True:
            dict = sys_API0001_3().nb(env_name, dict)
        ...
        try:
            ...
            if 'win' in sys.platform:   #windows系统平台
                if dict['logInfo']['code'] == '1':
                    print '是否出单(至复核结束)成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print '是否出单(至复核结束)成功：不成功'.decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否出单(至复核结束)成功：成功,,环境：%s,,投保日期：%s,,录入的代理人：%s,,录入的代理人：%s,,保单号：%s,,是否修改时间重启应用：%s(耗时：%s):,,新契约出单耗时：%s,,总耗时：%s" % (env_name, applicationDate, agent.strip(), partner.strip(), dict['policy_info']['policyNo'], dict['track_change'][-1]['trackData']['is_Modify_flag'], dict['track_change'][-1]['trackData']['consume_time'], API0000_diy().logger_count_time1(API0000_diy().logger_count_time(begin_time, end_time), dict['track_change'][-1]['trackData']['consume_time']), API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否出单(至复核结束)成功：不成功,,不成功原因：%s,,环境：%s,,投保日期：%s,,录入的代理人：%s,,录入的代理人：%s,,保单号：%s,,总耗时：%s" % (dict['logInfo']['msg'], env_name, applicationDate, agent.strip(), partner.strip(), dict['policy_info']['policyNo'], API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
