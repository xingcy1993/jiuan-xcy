...
class sys_API0034:
    ...
    def noticeConfig(self, apply_date, noticeConfigList, dict):
        """保全通知书寄送标识变更:
               step1:保全录入
               step2:保全复核
               step3:定义轨迹信息+检查点信息::   
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('保全通知书寄送标识变更_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            payment = 0
            #step1:保全录入
            rs = self.pa_entry(apply_date, noticeConfigList, dict)  
            resultData = rs[0]
            dict = rs[1]
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('保全通知书寄送标识变更_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
            ...
            if dict['logInfo']['code'] == '1':
                dict = self.save_check_info(apply_date, noticeConfigList, dict)
        return dict
    ...
    def pa_entry(self, apply_date, noticeConfigList, dict):
        """step1:保全录入
                1.保全录入用户登录系统
                2.保全申请接口
                3.保全录入-产品信息详细
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全通知书寄送标识变更_保全录入用户登录系统'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全通知书寄送标识变更_保全录入-客户受理信息接口'  # 节点
            message = ''
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全通知书寄送标识变更_保全录入-保全通知书寄送标识变更提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            applyTime1 = applyTime + " 00:00:00"  #组装申请日期applyTime
            #调用原子脚本
            response = API0000_pa().entryConfirm_csReversal(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime, serviceCode, noticeConfigList, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全通知书寄送标识变更_保全录入-登出系统'  # 节点
            message = ''
            #调用原子脚本
    ...
    def pa_review(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全通知书寄送标识变更_保全复核用户登录系统'  # 节点
            message = ''
            #dict获取数据+定义数据
            env_name = dict['public']['env_name']   #环境名称
            interface_ip = dict['public']['interface_ip']   #接口ip
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全通知书寄送标识变更_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全通知书寄送标识变更_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全通知书寄送标识变更_保全复核提交接口'  # 节点
            message = ''
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全通知书寄送标识变更_保全复核-登出系统'  # 节点
            message = ''
            #调用原子脚本
            rs = API0000_sys_otherapi().logout(magicX,interface_ip)
            #获取所需响应值
    ...
    def save_check_info(self, apply_date, noticeConfigList, dict):
        """step3:定义轨迹信息+检查点信息:
                1.定义轨迹信息
                2.定义检查点信息
        """ 
        try:        
            ...
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_noticeConfig = {'apply_date':apply_date, 'noticeConfigList':noticeConfigList, 'msg': ''}   
                dict['check']['calc_noticeConfig'].append(calc_noticeConfig) 
                """2.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'noticeConfig', 'trackTime':apply_date, 'trackData':{'noticeConfigList':noticeConfigList}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_noticeConfig = {'apply_date':apply_date, 'noticeConfigList':noticeConfigList, 'msg': message}   
                dict['check']['calc_noticeConfig'].append(calc_noticeConfig) 
                """2.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'noticeConfig', 'trackTime':apply_date, 'trackData':{'noticeConfigList':noticeConfigList}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            apply_date = '2021-06-30'   #保全申请日期
            noticeConfigApplyAmount = '100'    #保全通知书寄送标识变更金额
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','policyNo','apply_date','noticeConfigApplyAmount','username_entry','username_review']
                ...
                noticeConfigApplyAmount = rs[2][3]   #保全通知书寄送标识变更申请金额
                username_entry = rs[2][4]   #保全录入用户
                username_review = rs[2][5]   #保全复核用户
       #3.校验录入字段是否符合规范
        key_dict = {'env_name':env_name,'apply_date':apply_date,'noticeConfigApplyAmount':noticeConfigApplyAmount}   #录入字段中待校验字典
        if msg == '':
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo},'public': {'env_name':env_name, 'username_entry':username_entry, 'username_review':username_review, 'interface_ip':interface_ip, 'magicX':''}, 'logInfo': {'code':'0', 'msg':'', 'err':'', 'result':'', 'unit':''}, 'track_info': [], 'check':{'calc_noticeConfig':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            #time.sleep(30)
            dict = sys_API0034().noticeConfig(apply_date, noticeConfigApplyAmount, dict)
            assert dict['logInfo']['code'] == '1'   #断言是否异常
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否保全通知书寄送标识变更成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('是否保全通知书寄送标识变更成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否保全通知书寄送标识变更成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否保全通知书寄送标识变更成功：不成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,不成功原因：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
