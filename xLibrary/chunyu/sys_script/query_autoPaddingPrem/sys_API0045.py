    ...
    def query_sql(self, dict):
        try:
            ...
            if result == ():
                ...
                autoPaddingPrem = str(result[0][0])
                is_success = True
                message = ''
            # 加入断言
            assert is_success == True  

            if autoPaddingPrem == '' or autoPaddingPrem == 'None':
                autoPaddingPrem = '空'
            elif autoPaddingPrem == '0':
                autoPaddingPrem = '0-否'
            elif autoPaddingPrem == '1':
                autoPaddingPrem = '1-现金价值自垫'
            elif autoPaddingPrem == '2':
                autoPaddingPrem = '2-万能账户抵交'
            else:
                autoPaddingPrem = autoPaddingPrem + "(无此自垫方式)"
            dict['public']['autoPaddingPrem'] = autoPaddingPrem
...
if __name__ == "__main__":   
    try:
        ...
        #5.执行脚本
        if is_run == True:
            dict = sys_API0045().query_autoPaddingPrem(dict)
        ...
        try:
            ...
            if 'win' in sys.platform:   #windows系统平台
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "数据库查询保单自垫方式是否成功：成功,,环境：%s,,保单号：%s,,保单自垫方式为：%s,,耗时：%s" % (env_name, dict['public']['policyNo'], dict['public']['autoPaddingPrem'], API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "数据库查询保单自垫方式是否成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, dict['public']['policyNo'], API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
