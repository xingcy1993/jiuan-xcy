...
class sys_API0058:
    ...
    def query_sql(self, dict):
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('数据库根据职业等级查职业_开始(预计耗时:5s;)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            job_level = dict['public']['job_level']   #职业等级
            list = []
            ...
            #定义sql
            sql = "select a.code,a.name,b.name from d_job_category a,d_job_cate_2 b where a.parent_code=b.code and b.name='" + job_level + "';" 

            cursor.execute(sql)
            result = cursor.fetchall()
            if result == ():
                is_success = False
                message = env_name + '环境-数据库下查询不到此职业等级的职业！'
            else:
                is_success = True
                message = ''
            # 加入断言
            ...
            for i in range(0, len(result)):
                list.append({})
                job_code = str(result[i][0])   #职业编码
                job_name = str(result[i][1])     #职业名称
                job_level_name = str(result[i][2])   #职业等级名称

                #存入list
                list[-1]['job_code'] = job_code   #职业编码
                list[-1]['job_name'] = job_name   #职业名称
                list[-1]['job_level_name'] = job_level_name   #职业等级名称
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('数据库根据职业等级查职业_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
    ...
    <body>
        <table table border='1' bordercolor='black' cellspacing='0' cellpadding='5'>
            ...
            """%(list[j]['job_code'], list[j]['job_name'], list[j]['job_level_name'])
                file.write(message)
            file.write('\n' + '        </table> ' + "\n")
            file.write('    </body> ' + "\n")
            file.write('</html> ' + "\n")
    ...
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位:脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','db_user','db_password','job_level']
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'public':{'env_name':env_name, 'html_file_path':logging_filename, 'job_level':job_level, 'db_ip':db_ip,'db_user':db_user,'db_password':db_password},'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':''}}

        #5.执行脚本
        if is_run == True:
            dict = sys_API0058().query_job_level(dict)
        ...
        try:
            ...
            if 'win' in sys.platform:   #windows系统平台
                if dict['logInfo']['code'] == '1':
                    print '数据库根据职业等级查职业是否成功：成功'.decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('录入的职业等级：' + dict['public']['job_level']).decode('utf-8').encode('gb2312')
                    print ('耗时：' + API0000_diy().logger_count_time(begin_time, end_time)).decode('utf-8').encode('gbk')
                else:
                    print '数据库根据职业等级查职业是否成功：不成功'.decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "数据库根据职业等级查职业是否成功：成功,,环境：%s,,录入的职业等级：%s,,耗时：%s" % (env_name, dict['public']['job_level'],API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "数据库根据职业等级查职业是否成功：不成功,,不成功原因：%s,,环境：%s,,录入的职业等级：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, dict['public']['job_level'], API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
