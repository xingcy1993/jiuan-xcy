...
class sys_API0060:
    ...
    def query_pa_product(self, dict):
        try:
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
    ...
    def query_sql(self, dict):
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('数据库查询支持此保险项目的产品_开始(预计耗时:5s;)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            liab_name = dict['public']['liab_name']   #保全项目名称
            list = []
            ...
            #定义sql
            sql = "select b.product_code,a.product_id,b.product_name,if(b.main_rider='M','主险','附加险') from t_liability a,t_product b where a.product_id=b.product_id and a.liab_name='" + liab_name + "'" 

            cursor.execute(sql)
            result = cursor.fetchall()
            if result == ():
                is_success = False
                message = env_name + '环境-数据库下查询不到此保全项目名称！'
            else:
                is_success = True
                message = ''
            # 加入断言
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('数据库查询支持此保险项目的产品_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
    ...
    <body>
        <table table border='1' bordercolor='black' cellspacing='0' cellpadding='5'>
            ...
            """%(list[j]['product_code'], list[j]['product_id'], list[j]['product_name'], list[j]['main_rider'])
                file.write(message)
            file.write('\n' + '        </table> ' + "\n")
            file.write('    </body> ' + "\n")
            file.write('</html> ' + "\n")
    ...
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位:脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','db_user','db_password','liab_name']
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'public':{'env_name':env_name, 'html_file_path':logging_filename, 'liab_name':liab_name, 'db_ip':db_ip,'db_user':db_user,'db_password':db_password},'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':''}}

        #5.执行脚本
        if is_run == True:
            dict = sys_API0060().query_pa_product(dict)
        ...
        try:
            ...
            if 'win' in sys.platform:   #windows系统平台
                if dict['logInfo']['code'] == '1':
                    print '数据库查询支持此保险项目的产品是否成功：成功'.decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('录入的保全项目名称：' + dict['public']['liab_name']).decode('utf-8').encode('gb2312')
                    print ('耗时：' + API0000_diy().logger_count_time(begin_time, end_time)).decode('utf-8').encode('gbk')
                else:
                    print '数据库查询支持此保险项目的产品是否成功：不成功'.decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "数据库查询支持此保险项目的产品是否成功：成功,,环境：%s,,录入的保全项目名称：%s,,耗时：%s" % (env_name, dict['public']['liab_name'],API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "数据库查询支持此保险项目的产品是否成功：不成功,,不成功原因：%s,,环境：%s,,录入的保全项目名称：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, dict['public']['liab_name'], API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
