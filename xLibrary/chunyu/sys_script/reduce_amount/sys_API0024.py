...
class sys_API0024:
    ...
    def reduce_amount(self, apply_date, adjustAmount, dict):
        """系统减保:
               step1:保全录入
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '减保_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            payment = 0
            #1.保全录入
            rs = self.pa_entry(apply_date, adjustAmount, dict)  
            resultData = rs[0]
            dict = rs[1]
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '减保_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
    ...
    def pa_entry(self, apply_date, adjustAmount, dict):
        """step1:保全录入
                1.保全录入用户登录系统
                2.保全申请接口
                3.保全录入-产品信息详细
        ...
        try:
           ...
           #节点初始化
            is_success = ''
            unit = '系统-保全减保_保全录入-产品信息详细'  # 节点
            message = ''
            #dict获取数据+定义数据
            reduce_amount_dict = []   #reduce_amount_dict。存入产品的减保数据信息   
            #调用原子脚本
            response = API0000_pa().queryCsPolicyProduct1(magicX, interface_ip, policyNo, changeId, partitionIndi) 
            #获取所需响应值
            is_success = response['isSuccess']
                #定义减保列表。并存入产品id、产品现价、产品减保限额；
            if is_success:
                message = ''
                resultData = response['resultData']
                resultData1 = resultData
                for z in range(0, len(resultData1)):
                    resultData1[z]['applyTimeStr'] = apply_date
                    resultData1[z]['adjustAmount'] = str(adjustAmount)   #减保金额
            else:
                message = response['message']
                resultData = ''
                resultData1 = ''
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全减保_保全录入-保单减保计算接口'  # 节点
            message = ''
            #调用原子脚本
            response = API0000_pa().saveProductSaChange(magicX, interface_ip, resultData1)
            #获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
           ...
           #节点初始化
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全减保_保全录入-支付方式及支付账户接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            #调用原子脚本
            response = API0000_pa().entryConfirm_additional_premium(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime, serviceCode, csProductCals, payMode, accountId, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
    ...
    def pa_review(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全减保_保全复核用户登录系统'  # 节点
            message = ''
            #dict获取数据+定义数据
            env_name = dict['public']['env_name']   #环境名称
            interface_ip = dict['public']['interface_ip']   #接口ip
            username_entry = dict['public']['username_review']    #保全复核用户
            #调用原子脚本
            rs = API0000_sys_otherapi().login(env_name,username_entry, interface_ip)
            #获取所需响应值
            is_success = rs[0]
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全减保_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全减保_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
    ...
    def save_check_info(self, apply_date, payment, resultData, dict):
        ...
        try: 
            ...
            for i in range(0,len(resultData)):
                ...
                #减保后保险金额 
                if resultData[i]['changeSumAssured'] == None:
                    changeSumAssured = 0
                else:
                    changeSumAssured = float(resultData[i]['changeSumAssured'])
                #减保后保费金额   
                if resultData[i]['changeStdPremAf'] == None:
                    changeStdPremAf = 0
                else:
                    changeStdPremAf = float(resultData[i]['changeStdPremAf'])
                #减保后现金价值 
                if resultData[i]['changePresentPrice'] == None:
                    changePresentPrice = 0
                else:
                    changePresentPrice = float(resultData[i]['changePresentPrice'])
                #退还续期保费
                ...
                #每个产品减保检查点追加到product减保产品列表中
                product.append({'productCode':productCode, 'sumAssured':sumAssured, 'stdPremAf':stdPremAf, 'presentPrice':presentPrice, 'changeSumAssured':changeSumAssured, 'changeStdPremAf':changeStdPremAf, 'changePresentPrice':changePresentPrice, 'refundRenewPrem':refundRenewPrem}) 
                #总减保金额
                payment = payment + refundRenewPrem
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_reduce_amount = {'apply_date':apply_date, 'product':product, 'payment':payment, 'msg': ''}   
                dict['check']['calc_reduce_amount'].append(calc_reduce_amount)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'reduce_amount', 'trackTime':apply_date, 'trackData':{'product':product, 'payment':payment}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_reduce_amount = {'apply_date':apply_date, 'product':'', 'payment':payment, 'msg': message}   
                dict['check']['calc_reduce_amount'].append(calc_reduce_amount)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'reduce_amount', 'trackTime':apply_date, 'trackData':{'product':'', 'payment':payment}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":  
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088622045172368'
            apply_date = '2020-07-20'   #保全申请日期
            adjustAmount = '1000'   #减保金额
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','policyNo','apply_date','adjustAmount','username_entry','username_review']
                ...
                adjustAmount = rs[2][3]   #减保金额
                username_entry = rs[2][4]   #保全录入用户
                username_review = rs[2][5]   #保全复核用户
       #3.校验录入字段是否符合规范
        key_dict = {'env_name':env_name,'apply_date':apply_date,'adjustAmount':adjustAmount}   #录入字段中待校验字典
        if msg == '':
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo},'public': {'env_name':env_name, 'username_entry':username_entry, 'username_review':username_review, 'interface_ip':interface_ip, 'magicX':''}, 'logInfo': {'code':'0', 'msg':'', 'err':'', 'result':'','job':job}, 'track_info': [], 'check':{'calc_reduce_amount':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            dict = sys_API0024().reduce_amount(apply_date, adjustAmount, dict)
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否减保成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('减保金额：' + adjustAmount).decode('utf-8').encode('gb2312')
                    print ('耗时：' + API0000_diy().logger_count_time(begin_time, end_time)).decode('utf-8').encode('gbk')
                else:
                    print ('是否减保成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                    print ('减保金额：' + adjustAmount).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否减保成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,减保金额：%s,,耗时：%s" % (env_name, policyNo, apply_date, adjustAmount, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否减保成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,减保金额：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, adjustAmount, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
