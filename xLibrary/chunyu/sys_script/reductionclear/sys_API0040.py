...
class sys_API0040:
    ...
    def reductionclear(self, apply_date, dict):
        """系统减额交清:
               step1:保全录入
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '减额交清_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '减额交清_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
            ...
            if dict['logInfo']['code'] == '1':
                dict = self.save_check_info(apply_date, resultData, dict)
        return dict
    ...
    def pa_entry(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全减额交清_保全录入-客户受理信息接口'  # 节点
            message = ''
            #调用原子脚本
            ...
            assert is_success == True 
                #连接数据库
            unit = '系统-保全减额交清_保全录入-客户受理信息接口_连接数据库'
            ...
            for i in range(0,len(customerInfoList_old)):
                if customerInfoList_old[i]['customerRole'] == '01':
                    ...
                    unit = '系统-保全减额交清_保全录入-客户受理信息接口_数据库查询客户证件止期'
                    cursor.execute(sql)
           ...
           #节点初始化
            is_success = ''
            unit = '系统-保全减额交清_保全录入-减额交清信息查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            reductionclear_dict = []   #reductionclear_dict。存入产品的减额交清数据信息   
            #调用原子脚本
            response = API0000_pa().queryReductionClear(magicX, interface_ip, changeId, partitionIndi, policyNo, apply_date) 
            #获取所需响应值
            is_success = response['isSuccess']
                #定义减额交清列表。并存入产品id、产品现价、产品减额交清限额；
            if is_success:
                message = ''
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全减额交清_保全录入-减额交清提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            csProductCals = resultData
            #调用原子脚本
            response = API0000_pa().entryConfirm_reductionclear(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime, serviceCode, csProductCals, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
    ...
    def pa_review(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全减额交清_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全减额交清_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
    ...
    def save_check_info(self, apply_date, resultData, dict):
        """step3:定义轨迹信息+检查点信息: 
                 1.定义轨迹信息
                 2.定义检查点信息
        """
        try: 
            ...
            for i in range(0,len(resultData)):
                ...
                #保费缴至日  
                if resultData[i]['payDueDate'] == None:
                    payDueDate = 0
                else:
                    payDueDate = resultData[i]['payDueDate']
                #现金价值    
                ...
                #每个产品减额交清检查点追加到product减额交清产品列表中
                product.append({'productCode':productCode, 'sumAssured':sumAssured, 'totalPremAf':totalPremAf, 'payDueDate':payDueDate, 'presentPrice':presentPrice, 'loanAccountAmount':loanAccountAmount, 'changeSumAssured':changeSumAssured, 'changePresentPrice':changePresentPrice}) 
            
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_reductionclear = {'apply_date':apply_date, 'product':product, 'msg': ''}   
                dict['check']['calc_reductionclear'].append(calc_reductionclear)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'reductionclear', 'trackTime':apply_date, 'trackData':{'product':product}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_reductionclear = {'apply_date':apply_date, 'product':'', 'msg': message}   
                dict['check']['calc_reductionclear'].append(calc_reductionclear)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'reductionclear', 'trackTime':apply_date, 'trackData':{'product':''}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088633856325068'
            apply_date = '2023-10-01'   #保全申请日期
       ...
       #3.校验录入字段是否符合规范
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo':policyNo},'public': {'env_name':env_name, 'username_entry':username_entry, 'username_review':username_review, 'interface_ip':interface_ip, 'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'', 'job':job}, 'track_info': [], 'check':{'calc_reductionclear':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            dict = sys_API0040().reductionclear(apply_date, dict)
            assert dict['logInfo']['code'] == '1'   #断言是否异常
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否减额交清成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('是否减额交清成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否减额交清成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否减额交清成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
