...
class sys_API0030:
    ...
    @func_set_timeout(240)#设定函数超执行时间
    def reissue(self, apply_date, reissueReason, reissueType, dict):
        """系统保单补发:
               step1:保全录入
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '保单补发_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            dict['logInfo']['result'] = ''
            reissueReason = reissueReason[0:1]
            reissueType = reissueType[0:1]
            #保全录入
            dict = self.pa_entry(apply_date, reissueReason, reissueType, dict)  
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '保单补发_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
            ...
            if dict['logInfo']['code'] == '1':
                dict = self.save_check_info(apply_date, reissueReason, reissueType, dict)
        return dict


    @func_set_timeout(120)#设定函数超执行时间
    def pa_entry(self, apply_date, reissueReason, reissueType, dict):
        """step1:保全录入
                1.保全录入用户登录系统
                2.保全申请接口
                3.保全录入-客户受理信息接口 
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单补发_保全录入-客户受理信息接口'  # 节点
            message = ''
            #调用原子脚本
            ...
            assert is_success == True 
                #连接数据库
            unit = '系统-保全保单补发_保全录入-客户受理信息接口_连接数据库'
            ...
            unit = '系统-保全保单补发_保全录入-客户受理信息接口_数据库查询客户证件止期'
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单补发_保全录入-保单补发信息保存接口'  # 节点
            message = ''
            validationFlag = 'N'    
            #调用原子脚本
            response = API0000_pa().saveReissueInfo(magicX, interface_ip, validationFlag, changeId, partitionIndi, policyNo, reissueReason, reissueType)
            #获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单补发_保全录入-保单补发提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            applyTime = applyTime + " 00:00:00"  #组装申请日期applyTime
            #调用原子脚本
            response = API0000_pa().entryConfirm_suspend(magicX, interface_ip, changeId, applyNo, partitionIndi, policyId, policyNo, subSalesChannel, applyTime, serviceCode, customerInfoList)
            # 获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
            ...
            #如果程序报错，且magicX不为空（已经登录），对用户进行解锁
            if dict['public']['magicX'] != '':
                API0000_sys_otherapi().deblocking(dict['public']['magicX'], interface_ip, username_entry)
            #记录异常日志
            logging.warning(API0000_diy().text_conversion('&nbsp&nbsp&nbsp'*space_times + '*异常位置:\n')  + traceback.format_exc())
        finally:
    ...
    def pa_review(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单补发_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单补发_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
    ...
    def save_check_info(self, apply_date, reissueReason, reissueType, dict):
        """step3:定义轨迹信息+检查点信息: 
                 1.定义轨迹信息
                 2.定义检查点信息
        """
        try: 
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_reissue = {'apply_date':apply_date, 'reissueReason':reissueReason, 'reissueType':reissueType, 'msg': ''}   
                dict['check']['calc_reissue'].append(calc_reissue)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'reissue', 'trackTime':apply_date, 'trackData':{'reissueReason':reissueReason, 'reissueType':reissueType}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_reissue = {'apply_date':apply_date, 'msg': message}   
                dict['check']['calc_reissue'].append(calc_reissue)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'reissue', 'trackTime':apply_date, 'trackData':{}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088622045173768'   #保单号
            apply_date = '2021-06-03'   #保全申请日期
            reissueReason = '1-客户遗失'    #补发原因。1-客户遗失；2-业务员遗失；3-保单污损；4-其他
            reissueType =  '1-纸质保单'   #补发类型。1-纸质保单；2-电子保单
            ...
            if len(sys.argv) != 3:   #判读后台出参数量（3位：脚本路径+测试编号+json串）
                ...
                policyNo = ''   
                apply_date = ''   
                reissueReason = ''    #补发原因。1-客户遗失；2-业务员遗失；3-保单污损；4-其他
                reissueType =  ''   #补发类型。1-纸质保单；2-电子保单
                username_entry = ''  
                username_review = ''  
            elif len(sys.argv) == 3:
                ...
                #后台传值json串中字段列表
                key_list1 = ['env_name','policyNo','apply_date','reissueReason','reissueType','username_entry','username_review']
                ...
                reissueReason = rs[2][3]   #补发原因
                reissueType = rs[2][4]   #补发类型
                username_entry = rs[2][5]   #保全录入用户
                username_review = rs[2][6]   #保全复核用户
        #3.校验录入字段是否符合规范
        key_dict = {'env_name':env_name,'apply_date':apply_date,'reissueReason':reissueReason,'reissueType':reissueType}   #录入字段中待校验字典
        if msg == '':
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo': policyNo,'magicX':''},'public': {'env_name':env_name, 'username_entry': username_entry, 'username_review': username_review, 'interface_ip': interface_ip, 'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'','job':job}, 'track_info': [], 'check':{'calc_reissue':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            dict = sys_API0030().reissue(apply_date, reissueReason, reissueType, dict)
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否保单补发成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('补发原因：' + reissueReason).decode('utf-8').encode('gb2312')
                    print ('补发类型：' + reissueType).decode('utf-8').encode('gb2312')
                    print ('耗时：' + API0000_diy().logger_count_time(begin_time, end_time)).decode('utf-8').encode('gbk')
                else:
                    print ('是否保单补发成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                    print ('补发原因：' + reissueReason).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否保单补发成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,补发原因：%s,,补发类型：%s,,耗时：%s" % (env_name, policyNo, apply_date, reissueReason, reissueType, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否保单补发成功：不成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,补发原因：%s,,补发类型：%s,,不成功原因：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, reissueReason, reissueType, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
