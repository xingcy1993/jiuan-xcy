...
class sys_API0025:
    ...
    def suspend(self, apply_date, dict):
        """系统保单挂失:
               step1:保全录入
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('保单挂失_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('保单挂失_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
    ...
    def pa_entry(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单挂失_保全录入-客户受理信息接口'  # 节点
            message = ''
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单挂失_保全录入-保单挂失信息保存接口'  # 节点
            message = ''
            validationFlag = 'N'   
            suspendCause = '1'    #挂失原因
            #调用原子脚本
            response = API0000_pa().saveSuspendInfo(magicX, interface_ip, validationFlag, changeId, partitionIndi, policyId, suspendCause)
            #获取所需响应值
            is_success = response['success']
            if is_success:
                message = ''
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单挂失_保全录入-保单挂失提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
    ...
    def pa_review(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单挂失_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单挂失_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            """存入dict"""
            #保单挂失状态、挂失原因
            dict['policy_info']['suspend'] = 'Y'   #保单挂失状态。Y-保单挂失；N-保单为解挂
            dict['policy_info']['suspend_reason'] = '1'    #保单挂失原因（保单挂失）
    ...
    def save_check_info(self, apply_date, dict):
        ...
        try: 
            if dict['logInfo']['code'] == '1':
                #存入dict检查点中
                calc_suspend = {'apply_date':apply_date, 'suspend':'Y', 'msg': ''}   
                dict['check']['calc_suspend'].append(calc_suspend)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'suspend', 'trackTime':apply_date, 'trackData':{'suspend':'Y'}, 'msg': ''}
                ...
                #存入dict检查点中
                calc_suspend = {'apply_date':apply_date, 'suspend':'', 'msg': message}   
                dict['check']['calc_suspend'].append(calc_suspend)
                """1.定义轨迹信息"""
                #定义轨迹信息
                track_info = {'trackType':'suspend', 'trackTime':apply_date, 'trackData':{'suspend':''}, 'msg': message}
                dict['track_info'].append(track_info)   #存入轨迹信息
        except Exception, e:
            dict['logInfo']['code'] = '0'    #记录异常标识
            #记录异常日志
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088633856323668'   #保单号
            apply_date = '2021-08-01'   #保全申请日期
       ...
       #3.校验录入字段是否符合规范
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo': policyNo, 'suspend':'N', 'suspend_reason':''},'public': {'env_name':env_name, 'username_entry': username_entry, 'username_review': username_review, 'interface_ip': interface_ip, 'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'','job':''}, 'track_info': [], 'check':{'calc_suspend':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            dict = sys_API0025().suspend(apply_date, dict)
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否保单挂失成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('保单挂失原因：保单遗失-保全挂起').decode('utf-8').encode('gb2312')
                    print ('耗时：' + API0000_diy().logger_count_time(begin_time, end_time)).decode('utf-8').encode('gbk')
                else:
                    print ('是否保单挂失成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否保单挂失成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,保单挂失原因：保单遗失-保全挂起,,耗时：%s" % (env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否保单挂失成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
