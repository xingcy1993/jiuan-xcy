...
class sys_API0013:
    ...
    def query_data(self, dict):
        try:
            ...
            for i in range(0, len(resultData)):
                ...
                chargeAmount = resultData[i]['chargeAmount']   #手续费
                loanAccountAmount = resultData[i]['loanAccountAmount']   #欠款余额
                refundRenewPrem = resultData[i]['refundRenewPrem']   #退还续期保费
                cbSbAccount = resultData[i]['cbSbAccount']   #累计生息账户
                riskChargeFee = resultData[i]['riskChargeFee']   #退还未到期风险保费
                bonusAccount = resultData[i]['bonusAccount']   #红利累积生息账户
                loanAccountAmount = resultData[i]['loanAccountAmount']   #已分红金额扣回
                ...
                if chargeAmount == None:
                    chargeAmount = '0'

                if refundRenewPrem == None:
                    refundRenewPrem = '0'

                if cbSbAccount == None:
                    cbSbAccount = '0'

                if riskChargeFee == None:
                    riskChargeFee = '0'

                if bonusAccount == None:
                    bonusAccount = '0'
                ...
                list[-1]['chargeAmount'] = chargeAmount
                list[-1]['loanAccountAmount'] = loanAccountAmount
                list[-1]['refundRenewPrem'] = refundRenewPrem
                list[-1]['cbSbAccount'] = cbSbAccount
                list[-1]['riskChargeFee'] = riskChargeFee
                list[-1]['bonusAccount'] = bonusAccount
                list[-1]['loanAccountAmount'] = loanAccountAmount
    ...
    <body>
        <table table border='2' bordercolor='black' cellspacing='0' cellpadding='5'>
            ...
            """%(dict['public']['policyNo'], list[j]['productName'],list[j]['liabilityState'],list[j]['holderName'],list[j]['insuredName'],list[j]['sumAssured'],list[j]['totalPremAf'],list[j]['presentPrice'],list[j]['investAmount'],list[j]['chargeAmount'],list[j]['loanAccountAmount'],list[j]['refundRenewPrem'],list[j]['cbSbAccount'],list[j]['riskChargeFee'],list[j]['bonusAccount'],list[j]['loanAccountAmount'],list[j]['surrenderAmount'])
                file.write(message2)
...
if __name__ == "__main__":
    try:
        ...
        if 'win' in sys.platform:   #(1)win系统（本地）
            ...
            policyNo = '8088622045174268'   #保单号
            trial_day = '2021-06-07'   #试算日期
       ...
       #3.校验录入字段是否符合规范
        ...
        #6.执行脚本
        if is_run == True:
            dict = sys_API0013().get_actuarial_data(dict)
            assert dict['logInfo']['code'] == '1'   #断言是否异常
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '退保试算是否成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('退保试算是否成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('试算日期：' + trial_day).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "退保试算是否成功：成功,,环境：%s,,保单号：%s,,试算日期：%s,,耗时：%s" % (env_name, policyNo, trial_day, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "退保试算是否成功：不成功,,不成功原因：%s,,环境：%s,,,,保单号：%s,,试算日期：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, trial_day, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
