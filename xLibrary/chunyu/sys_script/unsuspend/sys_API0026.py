...
class sys_API0026:
    ...
    def unsuspend(self, apply_date, dict):
        """系统保单解挂:
               step1:保全录入
        ...
        try:
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('保单解挂_开始(预计耗时:60s)'))
            #初始化
            dict['logInfo']['code'] = '0'
            dict['logInfo']['msg'] = ''
            dict['logInfo']['err'] = ''
            ...
            #记录日志
            logging.warning(API0000_diy().text_conversion('保单解挂_结束(实际耗时:%s)' % API0000_diy().logger_count_time(begin_time, end_time)))
        except Exception, e:
            dict['logInfo']['code'] = '0'
    ...
    def pa_entry(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单解挂_保全录入-客户受理信息接口'  # 节点
            message = ''
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单解挂_保全录入-保单解挂提交接口'  # 节点
            message = ''
            #dict获取数据+定义数据
    ...
    def pa_review(self, apply_date, dict):
        ...
        try:
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单解挂_保全复核-保全公共工作池查询接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            policyNo = dict['policy_info']['policyNo']    #保单号
            #调用原子脚本
            ...
            #节点初始化
            is_success = ''
            unit = '系统-保全保单解挂_保全复核-工作池领取任务接口'  # 节点
            message = ''
            #dict获取数据+定义数据
            ...
            """存入dict"""
            #保单解挂状态、解挂原因
            dict['policy_info']['suspend'] = 'N'   #保单解挂状态。Y-保单挂失；N-保单为解挂
            dict['logInfo']['code'] = '1'   #记录无异常标识
            end_time = datetime.datetime.now()   #结束时间
            #记录日志
...
if __name__ == "__main__":
    try:
       ...
       #3.校验录入字段是否符合规范
        ...
        if is_run == True:
            msg = ''
        dict = {'execNo': execNo, 'policy_info':{'policyNo': policyNo, 'suspend':'Y'},'public': {'env_name':env_name, 'username_entry': username_entry, 'username_review': username_review, 'interface_ip': interface_ip, 'magicX':''}, 'logInfo': {'code':'0', 'msg':msg, 'err':'', 'result':'','job':''}, 'track_info': [], 'check':{'calc_suspend':[]}, 'track_change':[]}
        #5.1定义logging文件路径
        ...
        if is_run == True:
            ...
            assert dict['logInfo']['code'] == '1'   #断言是否异常
            dict = sys_API0026().unsuspend(apply_date, dict)
        ...
        try:
            ...
            if 'win' in sys.platform:
                if dict['logInfo']['code'] == '1':
                    print '是否保单解挂成功：成功'.decode('utf-8').encode('gb2312')
                    ...
                    print ('是否保单解挂成功：不成功').decode('utf-8').encode('gb2312')
                    print ('环境：' + env_name).decode('utf-8').encode('gb2312')
                    print ('保单号：' + policyNo).decode('utf-8').encode('gb2312')
                    print ('保全申请日期：' + apply_date).decode('utf-8').encode('gb2312')
                ...
                if dict['logInfo']['code'] == '1':
                    result_str = "是否保单解挂成功：成功,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                else:
                    result_str = "是否保单解挂成功：不成功,,不成功原因：%s,,环境：%s,,保单号：%s,,保全申请日期：%s,,耗时：%s" % (dict['logInfo']['msg'], env_name, policyNo, apply_date, API0000_diy().logger_count_time(begin_time, end_time))
                print result_str
            #打印终端结束符
            print '#'*3 + 'RESULT' + '#'*3
        except Exception, e:
